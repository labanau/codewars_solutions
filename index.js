...
const checkValidCurrencyCode = code => new Promise( (resolve, reject) => {
	...
	request(baseURL, (err, res, body) => {
		...
		if (!rates.hasOwnProperty(code[0]) || !rates.hasOwnProperty(code[1])) reject(new Error(`invalid currency code ${code}`))
		resolve(code)
	})
})

const getData = code => new Promise( (resolve, reject) => {
	request(`${baseURL}?base=${code[0]}&symbols=${code[1]}`, (err, res, body) => {
		if (err) reject(new Error('invalid API call'))
		resolve(body)
	})
})
